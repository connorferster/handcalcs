...
class ExpressionCodegen(ast.NodeVisitor):
    ...
    _bin_op_rules: dict[type[ast.operator], expressions.BinOpRule]
    ...
    def __init__(
        ...
        self._bin_op_rules = (
            expressions.SET_BIN_OP_RULES
            if use_set_symbols
            else expressions.BIN_OP_RULES
        )
        self._compare_ops = (
            expressions.SET_COMPARE_OPS
            if use_set_symbols
            else expressions.COMPARE_OPS
        )
    ...
    def _generate_sum_prod(self, node: ast.Call) -> str | None:
        ...
        name = helpers.extract_function_name_or_none(node)
    ...
    def _generate_zeros(self, node: ast.Call) -> str | None:
        ...
        if isinstance(node.args[0], ast.Tuple):
            dims = [helpers.extract_int_or_none(x) for x in node.args[0].elts]
            ...
            dim = helpers.extract_int_or_none(node.args[0])
    ...
    def _generate_identity(self, node: ast.Call) -> str | None:
        ...
        ndims = helpers.extract_int_or_none(node.args[0])
        if ndims is None:
            return None
    ...
    def visit_Call(self, node: ast.Call) -> str:
        """Visit a Call node."""
        func_name = helpers.extract_function_name_or_none(node)
        ...
        rule = (
            expressions.BUILTIN_FUNCS.get(func_name)
            if func_name is not None
            else None
        )

        if rule is None:
            rule = expressions.FunctionRule(self.visit(node.func))

        if rule.is_unary and len(node.args) == 1:
            # Unary function. Applies the same wrapping policy with the unary operators.
            precedence = expressions.get_precedence(node)
            arg = node.args[0]
            # NOTE(odashi):
            ...
            force_wrap_factorial = isinstance(arg, ast.Call) and (
                func_name == "factorial"
                or helpers.extract_function_name_or_none(arg) == "factorial"
            )
            # Note(odashi):
    ...
    def _wrap_operand(
        ...
        child_prec = expressions.get_precedence(child)
    ...
    def _wrap_binop_operand(
        ...
        child: ast.expr,
        parent_prec: int,
        operand_rule: expressions.BinOperandRule,
    ) -> str:
        ...
        if isinstance(child, ast.Call):
            child_fn_name = helpers.extract_function_name_or_none(child)
            rule = (
                expressions.BUILTIN_FUNCS.get(child_fn_name)
        ...
        if expressions.BIN_OP_RULES[type(child.op)].is_wrapped:
            return latex
    ...
    def visit_BinOp(self, node: ast.BinOp) -> str:
        """Visit a BinOp node."""
        prec = expressions.get_precedence(node)
    ...
    def visit_UnaryOp(self, node: ast.UnaryOp) -> str:
        ...
        latex = self._wrap_operand(node.operand, expressions.get_precedence(node))
        return expressions.UNARY_OPS[type(node.op)] + latex

    def visit_Compare(self, node: ast.Compare) -> str:
        """Visit a Compare node."""
        parent_prec = expressions.get_precedence(node)
    ...
    def visit_BoolOp(self, node: ast.BoolOp) -> str:
        ...
        op = f" {expressions.BOOL_OPS[type(node.op)]} "
