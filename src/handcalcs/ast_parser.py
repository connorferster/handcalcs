...
def ast_to_custom_list(node: ast.AST) -> Any:
    """
    ...
    if isinstance(node, ast.BinOp):
        ...
        left = ast_to_custom_list(node.left)
        op_name = type(node.op).__name__
        op = ARITHMETIC_OPS.get(op_name, op_name) # Get the operator name (e.g., 'Add', 'Mult')
        
        right = ast_to_custom_list(node.right)
    ...
    # --- Rule 1: Simplest case (e.g., variable names, constants) ---
        ...
        right = ast_to_custom_list(node.comparators[0])
        return deque([left, op, right])
    ...
    # --- Rule 3: If/Elif/Else block ---
        ...
        if_dict["test"] = ast_to_custom_list(node.test)
        
        # "body": The block of code inside the if (nested list of statements)
        if_dict["body"] = [ast_to_custom_list(item) for item in node.body]
        ...
        if node.orelse:
            ...
            if len(node.orelse) == 1 and isinstance(node.orelse[0], ast.If):
                 if_dict["orelse"] = ast_to_custom_list(node.orelse[0])
            else:
                # Standard `else` block or multiple statements in `orelse`
                if_dict["orelse"] = [ast_to_custom_list(item) for item in node.orelse]
        else:
    ...
    # --- Rule 4: For loop block ---
        ...
        for_dict["body"] = [ast_to_custom_list(item) for item in node.body]
        ...
        if isinstance(node.iter, ast.Name):
            ...
            for_dict["iter"] = ast_to_custom_list(node.iter)
    ...
    # --- Other important nodes (e.g., Assignments, List construction) ---
        ...
        target = ast_to_custom_list(node.targets[0])
        value = ast_to_custom_list(node.value)
...
def convert_source_to_custom_list(source_code: str) -> deque[Any]:
    ...
    return ast_to_custom_list(tree)