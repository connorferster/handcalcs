...
from handcalcs.parsing.linetypes import CalcLine
...
def test_calc_lines(basic_parser):
    ...
    assert basic_parser(source_3) == deque([
        CalcBlock(lines=deque([
            CalcLine(assigns=deque(["c"]), expression_tree=deque([
                deque([
                    FunctionBlock(
                        module_name="math", 
                        function_name="sin", 
