...
@singledispatch
def apply_hc_func(elem: Any, func: Callable, apply_numeric: bool = False):
    return func(elem)


@apply_hc_func.register
def apply_hc_func_list(hc_tree: list, func: Callable, apply_numeric: bool = False):
    print("Dispatched list")
    hc_tree_acc = []
    for elem in hc_tree:
        transformed = apply_hc_func(elem, func, apply_numeric)
...
@apply_hc_func.register
def apply_hc_func_calc_line(cl: CalcLine, func: Callable, apply_numeric: bool = False):
    print("dispatched calc line")
    assigns_acc = deque()
...
@apply_hc_func.register
def apply_hc_func_expr_line(el: ExprLine, func: Callable, apply_numeric: bool = False):
    print("dispatch expr line")
    expr_tree_acc = deque()
...
@apply_hc_func.register
def apply_hc_func_funcblock(fb: FunctionBlock, func: Callable, apply_numeric: bool = False):
    print("dispatch for block")
    lines_acc = deque()
...
@apply_hc_func.register
def apply_hc_func_forblock(fb: ForBlock, func: Callable, apply_numeric: bool = False):
    print("dispatch for block")
    lines_acc = deque()
    for elem in fb.lines:
        transformed = apply_hc_func(elem, func, apply_numeric)
...
@apply_hc_func.register
def apply_hc_func_ifblock(ib: IfBlock, func: Callable, apply_numeric: bool = False):
    print("dispatch if block")
    lines_acc = deque()
...
@apply_hc_func.register
def apply_hc_func_compblock(cb: ComprehensionBlock, func: Callable, apply_numeric: bool = False):
    assign_acc = deque()
...
@apply_hc_func.register
def apply_hc_func_comp(comp: ForBlock, func: Callable, apply_numeric: bool = False):
    assigns_acc = deque()
...
@apply_hc_func.register
def apply_hc_func_list_obj(list_obj: List, func: Callable, apply_numeric: bool = False):
    print("Dispatch deque")
...
@apply_hc_func.register
def apply_hc_func_tuple(tuple_obj: Tuple, func: Callable, apply_numeric: bool = False):
...
@apply_hc_func.register
def apply_hc_func_set(set_obj: Set, func: Callable, apply_numeric: bool = False):
    print("Dispatch tuple")
    elems = set_obj.elems
    acc = deque()
    for elem in elems:
...
@apply_hc_func.register
def apply_hc_func_dict(dict_obj: Dictionary, func: Callable, apply_numeric: bool = False):
    print("Dispatch dict")
...
@apply_hc_func.register
def apply_hc_func_deque(elems: deque, func: Callable, apply_numeric: bool = False):
    print("Dispatch deque")
    elems_acc = deque()
    for elem in elems:
        transformed = func(elem, apply_numeric)
